...
if __name__=="__main__":
    ...
    if glob.glob("result_final%s"%class_num)==[]:
        # os.system("mkdir result_final%s")
        os.makedirs("result_final%s"%class_num)
    ims_all=[x.split("/")[-1] for x in glob.glob(r"../Data/raw/*")]
    # print(glob.glob('..\\*'))
    # print(glob.glob(r"..\Data\raw\*"))
    # exit()
    total_num_pic=len(ims_all)
    for label,ims in t_final.items():
        path_t="result_final%s/%s"%(class_num,label)
        if glob.glob(path_t)==[]:
    ...
    for im in ims_all:
        ...
        shutil.copy(r"../Data/raw/%s"%(im),'%s'%rest_file_path)
